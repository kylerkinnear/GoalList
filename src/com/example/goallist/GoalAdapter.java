	...
	public View getView(int index, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(whoCalledOnMe).inflate(
					R.layout.view_holder_goal_item, null);

			holder = new ViewHolder();
			holder.goalName = (TextView) convertView
					.findViewById(R.id.holder_goal_name);
			holder.goalProgressText = (TextView) convertView
					.findViewById(R.id.holder_goal_progress_text);
			holder.goalProgressBar = (ProgressBar) convertView
					.findViewById(R.id.holder_goal_progress_bar);
		...
		String progressText = nextItem.getNumComplete() + "/"
				+ nextItem.getNumTotal() + " complete";
		holder.goalProgressText.setText(progressText);

		int progress;
		if (nextItem.getNumTotal() != 0) {
			double progressD = ((double) nextItem.getNumComplete())
					/ ((double) nextItem.getNumTotal()) * 100;
